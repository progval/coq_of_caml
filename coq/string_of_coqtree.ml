...
let rec string_of_structure : Coqtree.structure_item list -> string = function
    | [] -> "\n(* ** *)\n"
...
and string_of_structure_item (prefix : bool) : Coqtree.structure_item -> string = function
    | Coqtree.Definition (id, Coqtree.Types st) -> String.concat "" ["Definition "; id; " : "; String.concat "\n" (List.map string_of_type st)]
    ...
    | Coqtree.Comment s -> "(* " ^ s ^ " *)"
    | Coqtree.SubStructure l -> string_of_structure l
    | Coqtree.StructureType _ -> failwith "logic error."

let string_of_coqtree = function
    Coqtree.Structure l -> string_of_structure l
